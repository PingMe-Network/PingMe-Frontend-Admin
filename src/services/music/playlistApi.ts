...
export const playlistApi = {
    ...
    createPlaylist: async (data: CreatePlaylistRequest): Promise<PlaylistDto> => {
        const response = await axiosMusicClient.post<ApiResponse<PlaylistDto>>("/music-service/playlists", data);
        return response.data.data;
    },

    // Get all playlists for current user-management
    getPlaylists: async (): Promise<PlaylistDto[]> => {
        const response = await axiosMusicClient.get<ApiResponse<PlaylistDto[]>>("/music-service/playlists");
    ...
    getPlaylistDetail: async (id: number): Promise<PlaylistDetailDto> => {
        const response = await axiosMusicClient.get<ApiResponse<PlaylistDetailDto>>(`/music-service/playlists/${id}`);
        return response.data.data;
    },

    // Delete a playlist
    deletePlaylist: async (id: number): Promise<void> => {
        await axiosMusicClient.delete<ApiResponse<void>>(`/music-service/playlists/${id}`);
    },
    ...
    updatePlaylist: async (id: number, data: { name?: string; isPublic?: boolean }): Promise<PlaylistDto> => {
        const response = await axiosMusicClient.put<ApiResponse<PlaylistDto>>(`/music-service/playlists/${id}`, data);
        return response.data.data;
    },

    // Add a song to playlist
    addSongToPlaylist: async (playlistId: number, songId: number): Promise<{ alreadyExists: boolean }> => {
        const response = await axiosMusicClient.post<ApiResponse<{ alreadyExists: boolean }>>(`/music-service/playlists/${playlistId}/songs/${songId}`);
        return response.data.data;
    },

    // Remove a song from playlist
    removeSongFromPlaylist: async (playlistId: number, songId: number): Promise<void> => {
        await axiosMusicClient.delete<ApiResponse<void>>(`/music-service/playlists/${playlistId}/songs/${songId}`);
    },
    ...
    reorderPlaylist: async (playlistId: number, orderedSongIds: number[]): Promise<void> => {
        ...
        await axiosMusicClient.patch<ApiResponse<void>>(`/music-service/playlists/${playlistId}/songs/reorder`, payload);
    },
    ...
    getPublicPlaylists: async (page: number = 0, size: number = 20): Promise<PageResponse<PlaylistDto>> => {
        const response = await axiosMusicClient.get<ApiResponse<PageResponse<PlaylistDto>>>("/music-service/playlists/public", {
            params: { page, size }
        });
        return response.data.data;
    },
